<script lang="ts">
  ...
  // Replace-all save; ids are regenerated server-side, so reload afterwards.
  async function saveList(list: WebSearchProvider[]): Promise<string | null> {
    try {
      await webSearchProvidersSave(
        list.map((p) => ({ title: p.title, url: p.url, enabled: p.enabled })),
      );
      providers = await webSearchProvidersList();
      error = "";
      saved = true;
  ...
  function openEdit(index: number) {
    editingIndex = index;
    formTitle = providers[index].title;
    formUrl = providers[index].url;
    modalError = "";
    modalOpen = true;
  }
  ...
  async function saveProvider() {
    ...
    if (editingIndex === null) {
      updated.push({ id: "", title, url, enabled: true, position: updated.length });
    } else {
      updated[editingIndex] = { ...providers[editingIndex], title, url };
    }
      ...
      <div class="rules">
        {#each providers as provider, i (i)}
          <div class="rule-row">
            ...
            />
            <div class="rule-main">
              <span class="rule-title">{provider.title}</span>
